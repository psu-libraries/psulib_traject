...
RSpec.describe 'Macros' do
  ...
  describe '#extract_link_data' do
    context 'A record where indicator 2 is 0 and magic word is not in one of the label subfields' do
      ...
      let(:result1) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_1], 'leader' => leader)) }

      it 'produces a fulltext link' do
        expect(result1['full_links_struct']).to match ['{"prefix":"","text":"scholarsphere.psu.edu","url":"https://scholarsphere.ps' \
    ...
    context 'A record where indicator 1 and 2 is blank and magic word is not in one of the label subfields' do
      ...
      let(:result1) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_2], 'leader' => leader)) }
    ...
    context 'A record with an indicator 2 of 0 and magic word is in one of the label subfields' do
      ...
      let(:result3) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_3], 'leader' => leader)) }

      it 'produces a partial link' do
        expect(result3['partial_links_struct']).to match ['{"prefix":"Carrots executive summary peas","text":"library.columbia.edu","url":"http://library.columbi' \
    ...
    context 'A record with an indicator 2 of 1 and magic word is not in one of the label subfields' do
      ...
      let(:result9) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_9], 'leader' => leader)) }

      it 'only produces partial links, no full links' do
        expect(result9['full_links_struct']).to be_nil
        expect(result9['partial_links_struct']).to match ['{"prefix":"v.19-20 Sept.1939-May 1941","text":"archive.org",' \
    ...
    context 'A record with an indicator 2 of 1 and magic word is in one of the label subfields' do
      ...
      let(:result3) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_7], 'leader' => leader)) }

      it 'produces a partial link' do
        expect(result3['partial_links_struct']).to match ['{"prefix":"Just prefix","text":"library.columbia.edu","url":"http://library.columbia.' \
    ...
    context 'A record with multiple 856s, one with ind2 of 1 and other with ind2 2' do
      ...
      let(:result5) { indexer.map_record(MARC::Record.new_from_hash('fields' => url_856_5, 'leader' => leader)) }

      it 'produces 1 supplemental link and 1 partial link' do
        expect(result5['partial_links_struct']).to match ['{"prefix":"","text":"usacac.army.mil","url":"http://usacac.army.mil/CAC2' \
                                                          '/MilitaryReview/mrpast2.asp","notes":""}']
        expect(result5['suppl_links_struct']).to match ['{"prefix":"","text":"calldp.leavenworth.army.mil","url":"http://calldp.lea' \
    ...
    context 'A record with a fulltext link to Summon' do
      ...
      let(:result6) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_6], 'leader' => leader)) }

      it 'produces a fulltext link with text that drops summon\'s sub-domains' do
        expect(result6['full_links_struct']).to match ['{"prefix":"","text":"serialssolutions.com","url":"http://SK8ES4MC2L.search.' \
    ...
    context 'A record with a url prefix is not http or https' do
      ...
      let(:result4) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_4], 'leader' => leader)) }

      it 'doesn\'t produce any URL' do
        expect(result4['full_links_struct']).to be_nil
        expect(result4['partial_links_struct']).to be_nil
        expect(result4['suppl_links_struct']).to be_nil
      end
    end
    ...
    context 'A record with a url that has all subfields for a prefix, label and notes' do
      ...
      let(:result8) { indexer.map_record(MARC::Record.new_from_hash('fields' => [url_856_8], 'leader' => leader)) }

      it 'produce a link struct with a label, prefix and notes' do
        expect(result8['full_links_struct']).to match ['{"prefix":"v.7","text":"Electronic resource (PDF)","url":"http://purl.access.gpo.gov/GPO/LPS47374",' \
