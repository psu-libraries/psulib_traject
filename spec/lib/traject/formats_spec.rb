...
RSpec.describe MarcFormatProcessor do
  ...
  context 'with Book in leader byte 6 and byte 7' do
    let(:record) { 'format_leader6_book.mrc' }
  ...
  context 'with Video in leader byte 6 and byte 7' do
    let(:record) { 'format_leader6_video.mrc' }

    it { is_expected.to contain_exactly 'Microfilm/Microfiche' }
  end

  context 'with Thesis in leader byte 6 and byte 7' do
    let(:record) { 'format_leader6_thesis.mrc' }

    it { is_expected.to contain_exactly 'Microfilm/Microfiche' }
  end

  context 'with Archives in leader byte 6 and byte 7' do
    let(:record) { 'format_leader6_archives.mrc' }

    it { is_expected.to contain_exactly 'Microfilm/Microfiche' }
  end

  context 'with 007 formats' do
    let(:record) { 'format_007_maps.mrc' }
