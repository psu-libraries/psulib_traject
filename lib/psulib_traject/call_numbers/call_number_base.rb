...
module PsulibTraject::CallNumbers
  class CallNumberBase
    ...
    VOL_PATTERN         = /([\.:\/\(])?(n\.s\.?\,? ?)?[\:\/]?((index|ind)\s)?(#{VOL_PARTS}|#{MONTHS})[\. -\/]?\d+([\/-]\d+)?( \d{4}([\/-]\d{4})?)?( ?suppl\.?)?/i
    VOL_PATTERN_LOOSER  = /([\.:\/\(])?(n\.s\.?\,? ?)?[\:\/]?((index|ind)\s)?(#{VOL_PARTS}|#{MONTHS})[\. -]?\d+.*/i
    VOL_PATTERN_LETTERS = /([\.:\/\(])?(n\.s\.?\,? ?)?[\:\/]?((index|ind)\s)?(#{VOL_PARTS}|#{MONTHS})[\/\. -]?[A-Z]?([\/-][A-Z]+)?.*/i
